...
export default function ConversationMode({
  ...
  return (
    <div className="flex-1 flex flex-col relative w-full max-w-5xl mx-auto md:px-6 md:pb-6 h-full overflow-hidden select-none">
      {/* Top Half: Secondary Person (Rotated 180deg) */}
      <div className={`flex-1 flex flex-col justify-between p-6 rotate-180 transition-colors duration-500 ${recordingSource === 'secondary' ? 'bg-pulse/10' : 'bg-surface-low'} md:rounded-3xl shadow-[0_16px_32px_rgba(0,0,0,0.2)] z-10 relative`}>
      ...
      <div className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 top-1/2">
        <div className="relative w-32 h-32 md:w-40 md:h-40">
          ...
          <div className="relative w-full h-full rounded-full bg-[#120f2d] border-4 border-outline-variant/20 overflow-hidden flex flex-col shadow-[0_0_50px_rgba(0,0,0,0.5)]">
            ...
            {/* Bottom Half Button (for Primary) */}
            <button
      ...
      <div className={`flex-1 flex flex-col justify-between p-6 transition-colors duration-500 ${recordingSource === 'primary' ? 'bg-pulse/10' : 'bg-surface'} pb-28 md:pb-6 relative md:rounded-b-3xl`}>
        <div className="flex-1 flex flex-col items-center justify-center py-8 px-4 gap-6">
          ...
          <div className={`flex items-end justify-center gap-1.5 h-10 mt-2 transition-opacity duration-300 ${isRecording ? 'opacity-100' : 'opacity-30'}`}>
            {[0.6, 0.8, 1, 0.9, 0.8, 0.7, 0.5].map((opacity, i) => (
              <div 
                key={i}
                className={`w-1.5 rounded-full ${i % 2 === 0 ? 'bg-tertiary' : 'bg-primary'} ${isRecording ? 'animate-wave-bar' : ''}`}
                style={{ 
